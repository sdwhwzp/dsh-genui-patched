...
function normalizeNode(value, path, warnings) {
	...
	if (COMPONENT_SCHEMAS[type] === void 0) return value;
	...
	const toneEnum = COMPONENT_SCHEMAS[type].enums === void 0 ? void 0 : COMPONENT_SCHEMAS[type].enums.tone;
...
function normalizeGenuiSpec(value) {
	...
	else if (typeof out.type === "string") return {
...
**字段速查**（完整见 genui skill）：\`stat\` \`{"label","value","delta"?}\` · \`table\` \`{"columns":[...],"rows":[[...]],"types"?,"total"?:true,"details"?:[[...]],"filter"?:id,"export"?:true}\` · \`callout\` \`{"tone":"info|success|warning|error","title","content"}\` · \`progress\` \`{"value":0-100,"label"?,"variant"?,"target"?}\` · \`keyvalue\` \`{"pairs":[{"key","value"}]}\` · \`steps\` \`{"steps":[{"title","desc"?}]}\` · tone 按组件不同：\`badge\` success|warn|danger|accent、\`card\` info|success|warning|danger、\`hero\` accent|success|warning|danger（近义值会自动归一并记一条警告）
...
*/
const name = "@changfenhuang/dsh-genui";
